...
//GATEWAY INITIALIZE (guarded)
let stripe = null;
if (process.env.STRIPE_SECRET_KEY) {
  try {
    stripe = new Stripe(process.env.STRIPE_SECRET_KEY);
  } catch (err) {
    console.error('Stripe initialization failed:', err && err.message ? err.message : err);
    stripe = null;
  }
} else {
...
const placeOrderStripe = async (req, res, next) => {
  try {
    if (!stripe) return res.status(503).json({ success: false, message: 'Stripe not configured on server' });
    const { userId, address, amount, items } = req.body;
